...
Moreover, 
\begin{equation*}
  \ichol{\RMAT} \vone = \wrapParens{1+\wrapParens{\nstrat-1}\rho}^{-\halff} = c > 0.
\end{equation*}
From the monotonicity of \ichol{\RMAT}, the null and alternative hypotheses can be expressed
as 
\begin{equation*}
...
the $p$ values by $p \mapsto \logof{2\min\wrapParens{p,1-p}}$, and
...
fig_cap <- paste0('We repeat the P-P plot of \\figref{motherload_sims_hidden_plotz}, but in log-log space.  ',
                  'To check uniformity in both tails we compute $p_{*} = \\logof{2\\min\\wrapParens{p,1-p}}$, and plot against a log uniform law. ',
                  'Again, we plot the deviance of sample quantiles from the theoretical quantiles, ',
                  "with error bars around the $x$ axis. \\cite{velez2015modified} ",
                  'Simulations use the exact \\RMAT and \\pvsnr to compute the covariance matrix. ',
                  'The plot indicates that the p-values reported by the procedure are slightly smaller than they theoretically should be; ',
                  'given the log transform, one can interpret the plot as indicating the nominal p-values are around 1-2\\% smaller than they should be. ')
...
fig_cap <- paste0('P values from the conditional estimation procedure were computed using sample estimates of the covariance matrix, \\RMAT, ',
                  ...
                  'P values $p$ were transformed to $p_{*}=\\logof{2\\min\\wrapParens{p,1-p}}$, then the $p_{*}$ are Q-Q plotted against a log uniform law, ',
...
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ' simulations are transformed via $p_{*} = \\logof{2\\min\\wrapParens{p,1-p}}$, then plotted against a log uniform law using the subsampling procedure. ',
                  ...
                  'Without correcting for higher order moments, the nominal p values are larger than uniform, on the order of 5\\% larger; ',
                  'under the correction, however, the nominal p values are smaller than uniform, by as much as 10\\%. ')
...
fig_cap <- paste0('The empirical type I rate under the null hypothesis is plotted versus $\\rho$ ',
                  ...
                  'for three testing procedures: the conditional estimation, vanilla Bonferroni correction, ',
                  ...
                  'Tests were performed at the $0.05$ level, which appears to be maintained by the conditional ',
                  'estimation and fixed Bonferroni procedures but not by the regular Bonferroni procedure.  ',
