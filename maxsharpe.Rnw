...
\begin{equation*}
\Hyp[0] : \forall_i \psnr[i]=\psnr[0]\quad\mbox{versus}\quad \Hyp[1] :  \exists_i \psnr[i] > \psnr[0],
\end{equation*}
subject to the simple model of \eqnref{simple_RMAT} with $\rho \ge 0$.
The inverse square root of \RMAT, given in \eqnref{simple_RMAT_ichol}, is monotonic.
Moreover, 
\begin{equation*}
  \ichol{\RMAT} \vone = \oneby{\sqrt{1-\rho+\nlatf\rho}}.
\end{equation*}
...
\begin{equation*}
  \sqrt{\ssiz}\ichol{\RMAT}\svsr \approx \normlaw{\sqrt{\ssiz}\ichol{\RMAT}\pvsnr,\eye}.
\end{equation*}
From the monotonicity and the relation on $\ichol{\RMAT}\vone$,
the null hypothesis can then be expressed as
\begin{equation*}
\Hyp[0] : \forall_i 
  c \psnr[i] = c \psnr[0]\quad\mbox{versus}\quad \Hyp[1] :  \exists_i 
  c \psnr[i] > c \psnr[0],
\end{equation*}
for $c=\wrapParens{1+\wrapParens{\nlatf-1}\rho}^{-\halff}$.
The equivalence of the alternative hypothesis follows from positivity of the $c$.
...
\begin{equation}
  \label{eqn:chibstat}
  \bar{x}^2 = \sum_i \frac{\ssiz}{1+\wrapParens{\nlatf-1}\rho} \pospsq{\wrapParens{\ssr[i] - \psnr[0]}},
\end{equation}
...
First we test a single case of $\nlatf=\Sexpr{nlatf}$ using a correlation
...
We repeat this experiment \Sexpr{nsim} times and collect the \Sexpr{nsim}
p-values. 
In \figref{motherload_sims_hidden_plotz}, we present a P-P plot of those
P-values against the uniform law.
P-P plots for such sample sizes are hard to interpret unless they show
gross deviances from the uniform law. 
Instead we present a subsampled P-P plot, as suggested by V{\\'e}lez and Correa Morales. \\cite{velez2015modified} ",
The difference between empirical and theoretical p-values at vigintiles
are plotted, along with the error bars around the $x$ axis based on 
the theoretical density and sample size. 

That plot suggests that the p-values are indeed uniformly distributed.
To check the tail behavior of our computed p-values we transform
the $p$ values by $p \mapsto \log{2\min\wrapParens{p,1-p}}$, and
present a subsampled Q-Q plot against a log uniform law, in 
...
# 7e9fddf8-92e1-45d3-8da4-ae4dab5e2ee8 
...
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ...
                  'Instead we plot the deviance of sample quantiles from the theoretical quantiles, ',
                  "with error bars around the $x$ axis. \\cite{velez2015modified} ",
...
#mht_pvals <- 1 - pmaxsr(sim$simvals,df=nday-1,np=p,zeta=max(zeta),ope=1)
#data_frame(simvals=sim$simvals,mht_pvals=mht_pvals,ce_pvals=sim$pvals) %>%
...
#ph <- sim %>%
  #arrange(pvals) %>%
  #mutate(rown=seq_len(n())) %>%
  #filter((rown %% nevery) == 0) %>%
	#ggplot(aes(sample=pvals)) + 
	#stat_qq(distribution=qunif,alpha=0.15) +
  #geom_abline(intercept=0,slope=1,color='red') + 
  #labs(title='P-P plot of p-values from conditional estimation procedure')


alpha <- 0.05
zalp <- qnorm(alpha/2,lower.tail=FALSE)
ph <- modqq(sim$pvals,qnt_func=qunif,dens_func=dunif,k=19) %>%
  mutate(ymin=-zalp * serr,ymax=zalp * serr) %>%
  ggplot(aes(iii,delta,ymin=ymin,ymax=ymax)) + 
  geom_point() + 
  geom_errorbar(width=0.5) + 
  geom_hline(yintercept=0,linetype=2,alpha=0.5) + 
  labs(x='vigintile',y=expression(Delta),
       title='Large sample P-P plot, p-values from conditional estimation procedure')
print(ph)
@
<<'motherload_sims_log_plotz',dependson=c('motherload_sims_run'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=fig_cap>>=
# fba5ce14-4639-4cf6-aa20-ba27d9b98041 
suppressMessages({
	library(dplyr)
	library(ggplot2)
})

fig_cap <- paste0('We repeat the P-P plot of \\figref{motherload_sims_hidden_plotz}, but in log-log space.  ',
                  'To check uniformity in both tails we compute $p_{*} = \\log{2\\min\\wrapParens{p,1-p}}$, and plot against a log uniform law. ',
                  'Again, we plot the deviance of sample quantiles from the theoretical quantiles, ',
                  "with error bars around the $x$ axis. \\cite{velez2015modified} ",
                  'Simulations use the exact \\RMAT and \\pvsnr to compute the covariance matrix. ')

#limsy <- c(1e-6,5e-3)
#ph <- sim %>%
#  mutate(transp=2*pmin(pvals,1-pvals)) %>%
#  arrange(transp) %>%
#  mutate(rown=seq_len(n())) %>%
#  mutate(xvals=ppoints(n())) %>%
#  ggplot(aes(x=xvals,y=transp)) + 
#  geom_point(alpha=0.25) + 
#  scale_x_log10(limits=limsy) + scale_y_log10(limits=limsy) + 
#  geom_abline(intercept=0,slope=1,color='red') + 
#  labs(title='P-P plot of transformed p-values from conditional estimation procedure',
#       x='theoretical',y='sample')
#ph <- sim %>%
  #mutate(transp=2*pmin(pvals,1-pvals)) %>%
  #ggplot(aes(sample=transp)) + 
  #stat_qq(distribution=qunif,alpha=0.25) +
  #scale_x_log10(limits=limsy) + scale_y_log10(limits=limsy) + 
  #geom_abline(intercept=0,slope=1,color='red') + 
  #labs(title='P-P plot of transformed p-values from conditional estimation procedure',
       #x='theoretical',y='sample')
...
ph <- sim %>%
  mutate(logtransp=log(2*pmin(pvals,1-pvals))) %>%
  pull(logtransp) %>%
  modqq(qnt_func=qlogunif,dens_func=dlogunif,k=19) %>%
  mutate(ymin=-zalp * serr,ymax=zalp * serr) %>%
  ggplot(aes(iii,delta,ymin=ymin,ymax=ymax)) + 
  geom_point() + 
  geom_errorbar(width=0.5) + 
  geom_hline(yintercept=0,linetype=2,alpha=0.5) + 
  labs(x='vigintile',y=expression(Delta),
       title='Large sample P-P plot, transformed p-values from conditional estimation procedure')
print(ph)
@
%UNFOLD
...
<<'causal_sims_log_plotz',dependson=c('causal_sims'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=fig_cap>>=

# b6aa6d8c-2872-4496-a704-a7e090645a7d 
fig_cap <- paste0('P values from the conditional estimation procedure were computed using sample estimates of the covariance matrix, \\RMAT, ',
                  'and the vector of \\txtSNRs, \\pvsnr. ',
                  'P values $p$ were transformed to $p_{*}=\\log{2\\min\\wrapParens{p,1-p}}$, then the $p_{*}$ are Q-Q plotted against a log uniform law, ',
                  'using the subsampling procedure. ')
suppressMessages({
	library(dplyr)
	library(ggplot2)
})
#limsy <- c(1e-6,5e-3)
#ph <- causim %>%
	#ggplot(aes(sample=2*pmin(pvals,1-pvals))) +
	#stat_qq(distribution=qunif,alpha=0.25) +
  #scale_x_log10(limits=limsy) + scale_y_log10(limits=limsy) + 
  #geom_abline(intercept=0,slope=1,color='red') + 
  #labs(title='P-P plot of transformed p-values from conditional estimation procedure, feasible estimates')
alpha <- 0.05
zalp <- qnorm(alpha/2,lower.tail=FALSE)
ph <- causim %>%
  mutate(logtransp=log(2*pmin(pvals,1-pvals))) %>%
  ...
  geom_hline(yintercept=0,linetype=2,alpha=0.5) + 
  labs(x='vigintile',y=expression(Delta),
       title='Large sample P-P plot, transformed p-values from conditional estimation procedure, feasible estimates')
print(ph)
@
%UNFOLD
...
we let $\nlatf$ vary from $\Sexpr{min(params$nlatf)}$ to $\Sexpr{max(params$nlatf)}$;
...
fig_cap <- paste0('Kolmogorov-Smirnov statistics summarizing uniformity of the test statistic $u$ are plotted versus $\\ssiz$ with facets for $\\nlatf$ and $\\rho$.  ',
                  'Broadly we see that the test statistic is less uniform in the regime where $\\nlatf\\gg\\ssiz$, but that a large positive $\\rho$ perhaps reduces the number of assets in that relationship.')
...
\eqnref{apx_srdist_gaussian} to account for non-normal returns.\todo{fix this description...}
...
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ' simulations are transformed via $p_{*} = \\log{2\\min\\wrapParens{p,1-p}}$, then plotted against a log uniform law using the subsampling procedure. ',
                  ...
                  'Once again we focus on the lower left corner of the plots to trim the plot size.')

# 155290d7-d763-4034-b068-0d68e6d874db 
suppressMessages({
	library(dplyr)
	library(ggplot2)
})
...
#ph <- rbind(ok_tsim %>% mutate(higher_order=TRUE),
  ...
  #mutate(quasi_p=2*pmin(pvals,1-pvals)) %>%
	#ggplot(aes(sample=quasi_p)) + 
	#stat_qq(distribution=qunif,alpha=0.25) +
  #geom_abline(intercept=0,slope=1,color='red') + 
  #scale_x_log10(limits=limsy) + scale_y_log10(limits=limsy) + 
  #facet_grid(.~higher_order,labeller=label_both) + 
  #labs(title='P-P plot of p-values from conditional estimation procedure')
...
ph <- rbind(ok_tsim %>% mutate(higher_order=TRUE),
  ...
  group_by(higher_order) %>%
    summarize(fooret=list(modqq(logtransp,qnt_func=qlogunif,dens_func=dlogunif,k=19))) %>%
  ungroup() %>%
  tidyr::unnest() %>%
  mutate(ymin=-zalp * serr,ymax=zalp * serr) %>%
  ggplot(aes(iii,delta,ymin=ymin,ymax=ymax,color=higher_order)) + 
  geom_point() + 
  geom_errorbar(width=0.5) + 
  geom_hline(yintercept=0,linetype=2,alpha=0.5) + 
  labs(x='vigintile',y=expression(Delta),
...
bot_rawsim <- function(nday,nlatf,one_zeta,nsim=100,rho=0,
  ...
  mhtpvals <- replicate(nsim,{
    ...
    xm <- which.max(x)
    pval <- SharpeR::psr(max(x),df=nday-1,zeta=0,ope=1,lower.tail=FALSE) 

    # chibarsquare test, assuming rho=0
    chibs <- (nday) * sum(pmax(x,0)^2)
    chibp <- sum(sbwt * pchisq(chibs,df=sbdf,lower.tail=FALSE))

    c(pval,chibp,mu[xm])
  })
  rbind(sim %>% select(-simvals) %>% mutate(pvals=1-pvals) %>% mutate(method='condest'),
        data_frame(pvals=nlatf * as.numeric(mhtpvals[1,]),  # the pvals here is a correction for mht, a hack
                   popvals=as.numeric(mhtpvals[3,]),
                   method='mht'),
        data_frame(pvals=as.numeric(mhtpvals[2,]),  # no correction here for chibar square
                   popvals=as.numeric(mhtpvals[3,]),
                   method='chibs'))
}
...
<<'power_plotz_one',dependspn=c('sims_vs_mht'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=paste0('The empirical power of the conditional estimation and MHT corrected tests are shown versus the \\txtSNR of the asset with maximum \\txtSR under different arrangements of the vector \\pvsnr.')>>=
# 482b4da5-36ad-45fe-9c8f-687625aa5cc7 
#filter(grepl('equal|onegood',mutype)) %>%
library(ggplot2)
ph <- resu %>%
  mutate(method=gsub('^condest$','cond. est.',method)) %>%
  mutate(method=gsub('^mht$','MHT',method)) %>%
  mutate(method=gsub('^chibs$','chi-bar^2',method)) %>%
...
<<'rho_plotz_one',dependson=c('correlation_sims_vs_mht'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=paste0('The empirical type I rate under the null hypothesis is plotted versus $\\rho$ for the case where $\\RMAT=\\makerho{\\rho}{\\wrapParens{1-\\rho}}$, for three testing procedures: the conditional estimation, vanilla Bonferroni correction, and Bonferroni correction with fix for common correlation.  Tests are performed with Gaussian returns, for ',mht_nlatf,' assets over ',mht_nday,' days. Tests were performed at the $0.05$ level, which appears to be maintained by the conditional estimation and fixed Bonferroni procedures but not by the regular Bonferroni procedure.  Empirical rates are over ',mht_null_nsim,' simulations. The $y$ axis is in log scale.')>>=
#85d1c8a5-0c34-4217-95f9-1613751d84e9 
library(ggplot2)
ph <- rhoresu %>%
  mutate(method=gsub('^nelc_pvals$','cond. est.',method)) %>%
  mutate(method=gsub('^bonf_pvals$','vanilla MHT',method)) %>%
  mutate(method=gsub('^corr_pvals$','fixed MHT',method)) %>%
...
\Sexpr{names(X)[1]} with a \txtSR of 
...
intervals on the \txtSNR of the \Sexpr{names(X)[1]} portfolio. 
