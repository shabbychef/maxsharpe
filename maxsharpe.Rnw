...
As a further check, in \figref{motherload_sims_log_plotz}, we present
a subsampled Q-Q plot or log transformed p values against a log uniform law.
...
fig_cap <- paste0('We repeat the P-P plot of \\figref{motherload_sims_hidden_plotz}, but plot quantiles of the log p values against a log uniform law. ',
                  'Again, we plot the deviance of sample quantiles from the theoretical quantiles, ',
                  "with error bars around the $x$ axis. \\cite{velez2015modified} ",
                  'Simulations use the exact \\RMAT and \\pvsnr to compute the covariance matrix. ',
                  'The plot indicates that the smallest p-values reported by the procedure are slightly smaller than they theoretically should be; ',
                  'given the log transform, one can interpret the plot as indicating the nominal p-values are around 3\\% smaller than they should be. ')
...
In \figref{causal_sims_log_plotz} we present a sampled P-P plot of the log p values
against a log uniform law.
...
fig_cap <- paste0('P values from the conditional estimation procedure were computed using sample estimates of the covariance matrix, \\RMAT, ',
                  ...
                  'Differences in empirical and theoretical vigintiles of log p values are plotted. ')
suppressMessages({
	library(dplyr)
	library(ggplot2)
})
...
In \figref{causal_tfive_sims_plotz} we present the subsampled P-P plots
...
though without the correction, the smallest p-values are slightly smaller
than nominal, while with the correction they are slightly larger.
...
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ' simulations are logged, then plotted against a log uniform law using the subsampling procedure. ',
                  ...
                  'Without correcting for higher order moments, the nominal p values are smaller than uniform, on the order of 6\\% larger; ',
                  'under the correction, however, the nominal p values are larger than uniform, by as much as 3\\%. ')
...
But note that the null hypothesis that MHT and one-sided test are testing
...
%all the relevant differences between the tests or their
