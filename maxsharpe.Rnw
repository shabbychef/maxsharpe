...
bot_rawsim <- function(nday,nstrat,one_zeta,nsim=100,rho=0,
                       ...
                       mutype=c('equal','onegood','halfgood','continuous_uniform','uniform','bellshaped')) {
...
<<'sims_vs_mht',echo=FALSE,eval=TRUE,cache=TRUE,dependson=c('motherload_sim_funcs','runtime')>>=
suppressMessages({
  library(tidyr)
})
...
<<'sims_vs_mht_two',echo=FALSE,eval=TRUE,cache=TRUE,dependson=c('motherload_sim_funcs','runtime')>>=
suppressMessages({
  library(tidyr)
})
...
plan(sequential)
@
<<'resu_two_summary',dependson=c('sims_vs_mht_two'),echo=FALSE>>=
one_zeta <- resu_two %>%
...
gen_mu <- function(nstrat,one_zeta,
                       uni_chunks=14,  # for uniform
                       bell_chunks=32,   # for bellshaped, but we get far fewer
                       mutype=c('equal','onegood','twogood','halfgood','continuous_uniform','uniform','bellshaped')) {
	mutype <- match.arg(mutype)
  mu <- switch(mutype,
               equal=rep(one_zeta,nstrat),
               onegood=c(one_zeta,rep(-one_zeta,nstrat-1)),
