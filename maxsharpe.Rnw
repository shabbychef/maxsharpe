...
mht_nsim <- ceiling(5e4/max(1,RUNTIME_PARAM))
mht_nday <- ceiling(252 * (1 + (3/max(1,RUNTIME_PARAM))))
...
# actual run the sims
params <- data_frame(one_zeta=seq(0.0025,1.025/sqrt(ope),length.out=10)) %>%
  tidyr::crossing(data_frame(mutype=check_types))

mht_fdr_nsim <- ceiling(2e4/max(1,RUNTIME_PARAM))
mht_nday <- ceiling(252 * (1 + (3/max(1,RUNTIME_PARAM))))
mht_nstrat <- ceiling(25 + (80/max(1,RUNTIME_PARAM)))
...
% go.
<<'plot_fdr',dependson=c('collect_sims_fdr','plot_funcs'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=tfg_width,fig.height=tfg_height,out.width=tot_width,out.height=tot_height,fig.pos='h',fig.cap=fig_cap>>=
...
@
<<'plot_fdr_II',dependson=c('collect_sims_fdr','plot_funcs'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=tfg_width,fig.height=tfg_height,out.width=tot_width,out.height=tot_height,fig.pos='h',fig.cap=fig_cap>>=
fig_cap <- paste0("The empirical positive False Discovery Rate (pFDR) of the ",
                  "conditional estimation and MHT procedures ",
                  "are shown versus the maximum population \\txtSNR for various arrangements of the true \\txtSNR. ",
                  "The pFDR is defined as the probability that the asset with maximum \\txtSR is null ($\\zeta \\le 0$) ",
