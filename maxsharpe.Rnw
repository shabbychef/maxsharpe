...
bot_rawsim <- function(nday,nstrat,one_zeta,nsim=100,rho=0,
  ...
  rbind(sim %>% 
        ...
        data_frame(pvals=as.numeric(mhtpvals[1,]),
                   popvals=as.numeric(mhtpvals[4,]),
                   method='mht'),
        data_frame(pvals=as.numeric(mhtpvals[2,]),
                   popvals=as.numeric(mhtpvals[4,]),
                   method='chibs'),
        data_frame(pvals=as.numeric(mhtpvals[3,]),
                   popvals=as.numeric(mhtpvals[4,]),
                   method='follman'))
}
...
fig_cap <- paste0("The empirical power of the conditional estimation, MHT corrected test, chi-bar-square test, and Follman's test ",
                  'and one-sided test are shown versus the \\txtSNR of the asset with maximum \\txtSR under different arrangements of the vector \\pvsnr. ',
