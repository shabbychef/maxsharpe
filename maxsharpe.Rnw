...
old_theme <- theme_set(theme_minimal(base_size=12) %+replace% theme(legend.position="bottom"))
...
\author{Steven E. Pav \thanks{\href{https://orcid.org/0000-0002-4197-6195}{\normalfont\texttt{ORCID: 0000-0002-4197-6195}}; \email{steven@gilgamath.com}.
...
We repeat this experiment \Sexpr{scales::comma(nsim)} times and collect the resultant putative p-values.
...
performing $\Sexpr{scales::comma(nsim)}$ simulations, 
setting $\nstrat=\Sexpr{nstrat}$,
$\RMAT=\makerho{\Sexpr{rho}}{\Sexpr{1-rho}}$.
...
$\Sexpr{scales::comma(ks_nsim)}$ simulations, computing p values from the feasible estimator.
...
Again we perform $\Sexpr{scales::comma(nsim)}$ simulations with
$\nstrat=\Sexpr{nstrat}$,
$\RMAT=\makerho{\Sexpr{rho}}{\Sexpr{1-rho}}$,
$\ssiz=\Sexpr{nday}\dayto{}$, \etc
...
For each setting we perform $\Sexpr{scales::comma(mht_nsim)}$ simulations then 
...
We have performed
$\Sexpr{scales::comma(mht_nsim)}$ simulations for each setting of the `good'
...
occuring even once over the $\Sexpr{scales::comma(mht_nsim)}$ simulations performed
for each setting. 
...
$\ssiz=\Sexpr{mht_nday}$, and perform $\Sexpr{scales::comma(mht_null_nsim)}$ simulations to estimate the 
