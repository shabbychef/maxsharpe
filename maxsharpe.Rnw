...
\begin{tikzpicture}[
    ...
    process/.style={rectangle, draw, thick, minimum width=3.0cm, minimum height=0.8cm, align=center},
    outcome/.style={rectangle, draw, thick, minimum width=2.1cm, minimum height=1.0cm, align=center},
    arrow/.style={-Stealth, thick}
]

% Top Flow
\node[process] (specify) {SPECIFY STRATEGIES};
\node[process] (all_bad) [below left=1.0cm and -0.5cm of specify] {ALL\\STRATEGIES\\ARE BAD};
\node[process] (some_ok) [below right=1.0cm and -0.5cm of specify] {SOME\\STRATEGIES\\ARE GOOD};
...
\node[process] (bad_ok) [below left=0.7cm and -2.1cm of backtest_ok] {SELECTED\\STRATEGY\\IS BAD};
\node[process] (good_ok) [below right=0.7cm and -1.9cm of backtest_ok] {SELECTED\\STRATEGY\\IS GOOD};

% Final Outcomes
\node[outcome] (fail_bad_bad) [below left=0.7cm and -1.0cm of bad_bad] {TEST FAILS\\TO REJECT\\NULL};
\node[outcome] (rej_bad_bad) [below right=0.7cm and -1.95cm of bad_bad] {TEST\\REJECTS\\NULL};

\node[outcome] (fail_bad_ok) [below left=0.7cm and -1.3cm of bad_ok] {TEST FAILS\\TO REJECT\\NULL};
\node[outcome] (rej_bad_ok) [below right=0.7cm and -1.65cm of bad_ok] {TEST\\REJECTS\\NULL};
\node[outcome] (fail_good_ok) [below left=0.7cm and -2.5cm of good_ok] {TEST FAILS\\TO REJECT\\NULL};
\node[outcome] (rej_good_ok) [below right=0.7cm and -0.45cm of good_ok] {TEST\\REJECTS\\NULL};
