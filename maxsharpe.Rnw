...
\svsr \approx \normlaw{\pvsnr,\oneby{\ssiz}\RMAT}.
\end{equation*}
As in \eqnref{simple_RMAT_ichol}, the inverse square root of \RMAT is monotonic and
takes the form
\begin{equation*}
  \ichol{\RMAT} = \makerho{b_1}{b_2},
\end{equation*}
for some $b_2 > 0$, with $0 \ge b_1 > -b_2 / \nlatf$.

Given that, we can transform \svsr as
\begin{equation*}
...
\begin{equation}
  \label{eqn:chibstat}
  \bar{x}^2 = \sum_i \ssiz \wrapParens{\nlatf b_1 + b_2}^2 \pospsq{\wrapParens{\ssr[i] - \psnr[0]}},
\end{equation}
...
In testing we will see that the one-sided test is highly susceptible to distribution of the \pvsr,
...
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ...
                  "with error bars around the $x$ axis, as suggested by V{\\'e}lez and Correa Morales. \\cite{velez2015modified} ",
...
fig_cap <- paste0('We repeat the P-P plot of \\figref{motherload_sims_hidden_plotz}, but in log-log space.  ',
                  'To check uniformity in both tails we compute and plot $p_{*}=2\\min\\wrapParens{p,1-p}$ versus a uniform distribution.  ',
                  'To reduce plot size, we zoom in on the lower left corner of the P-P plot.')
...
<<'many_sims_plotz',dependson=c('many_sims'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap='Kolmogorov-Smirnov statistics summarizing uniformity of the test statistic $u$ are plotted versus $\\ssiz$ with facets for $\\nlatf$ and $\\rho$.  Broadly we see that the test statistic is less uniform in the regime where $\\nlatf\\gg\\ssiz$, but that a large positive $\\rho$ perhaps reduces the number of assets in that relationship.'>>=
...
#2FIX: fix this description
fig_cap <- paste0('The computed p-values from the conditional estimation procedure over ',nsim,
                  ' simulations are plotted against a uniform law.  Returns are drawn from a $\\tstat{}\\wrapParens{5}$ distribution.  ',
...
bot_rawsim <- function(nday,nlatf,one_zeta,nsim=100,rho=0,
  ...
  mhtpvals <- replicate(nsim,{
    ...
    chibp <- sum(cwts * pchisq(chibs,df=0:nlatf,lower.tail=FALSE))

    c(pval,chibp,mu[xm])
  })
  rbind(sim %>% select(-simvals) %>% mutate(pvals=1-pvals) %>% mutate(method='condest'),
        data_frame(pvals=nlatf * as.numeric(mhtpvals[1,]),
                   popvals=as.numeric(mhtpvals[3,]),
                   method='mht'),
        data_frame(pvals=nlatf * as.numeric(mhtpvals[2,]),
                   popvals=as.numeric(mhtpvals[3,]),
                   method='chibs'))
}
