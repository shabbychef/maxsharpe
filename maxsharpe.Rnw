...
RUNTIME_PARAM <- max(10,as.numeric(Sys.getenv('RUNTIME_PARAM')),na.rm=TRUE)
@
...
# 2FIX: want "\\rho" in the facet labels somehow. cf https://sahirbhatnagar.com/blog/2016/02/08/ggplot2-facet-wrap-labels/
require(latex2exp)
...
the ``half-good'' alternative. We also consider the cases where the elements of \psnr are
...
and the conditional procedure, which has persistently lower power than the others
for these two alternatives.


<<'power_plotz_one',dependson=c('sims_vs_mht','plot_funcs'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=fig_cap>>=
...
be conditioning on ``the asset with the largest \txtSR, whichever one that is.''
...
\txtSNR; in some small number of them for the half-good
...
This is the down side of the apparently higher power of the tests based on the MHT:
they do not maintain the nominal rate for the conditional null and thus can exhibit 
higher than nominal type I rate.
However, it should be rare that the selected asset is `bad' while other assets are actually `good'.
...
It is unlikely that the three distributions of $\pvsnr$ considered above will be encountered in real world applications;
...
We note that in some unlucky cases $\psnr[1] \le 0$ even though the maximum element of \pvsnr is
...
2FIX: \emph{rewrite this a little...}
