...
alt_test <- function(nday,mu,nsim=100,rho=0,psnr0=0,do_condest=TRUE) {
  nstrat <- length(mu)
  R <- pmin(diag(nstrat) + rho,1)  
  kurty <- 1.0 # assumed
  ...
  if (do_condest) {
    cd_sim <- parsim(nsim=nsim,nday=nday,R=R,mu=mu,testmu=rep(psnr0,nstrat)) %>%
      select(-simvals) %>% 
      mutate(pvals=1-pvals) %>% 
      mutate(method='condest')

    retv <- bind_rows(retv,cd_sim)
  }
  retv
}
...
bot_rawsim <- function(nday,nstrat,one_zeta,nsim=100,rho=0,
  ...
  alt_test(nday=nday,mu=mu,nsim=nsim,rho=rho,do_condest=do_condest)
}
