...
\subsection{Hansen's $\log \log$ Test}%FOLDUP
...
The idea is elegant, and ultimately simple to implement.  \cite{Hansen2003Asymptotic}
...
type I rate asymptotically, while achieving higher power than the chi-bar-square test.

For our problem, first perform the translation to \stvsr given in \eqnref{xi_transform}.
Then compute the $\bar{x}^2$ statistic of \eqnref{chibstat}.
However, instead of comparing to the unconstrained chi-bar-square statistic,
a different procedure is used to compute the rejection region.
...
bot_rawsim <- function(nday,nstrat,one_zeta,nsim=100,rho=0,
  ...
  sim <- parsim(nsim=nsim,nday=nday,R=R,mu=mu,testmu=rep(0,nstrat))
  ...
  # Hansens cutoff
  spcut <- - sqrt(2*log(log(nday))/nday)
  ...
  mhtpvals <- replicate(nsim,{
    ...
    # the nstrat is the MHT correction here, a Bonferroni hack.
    pval <- nstrat * SharpeR::psr(ssrmax,df=nday-1,zeta=0,ope=1,lower.tail=FALSE) 
    ...
    # hansens procedures; only for the rho=0 case, btw
    nabove <- sum(svsr > spcut)
    lval <- ifelse(nabove > 0,nabove * SharpeR::psr(ssrmax,df=nday-1,zeta=0,ope=1,lower.tail=FALSE),1)
    spval <- ifelse(nabove > 0,chib_test(chibs,nabove),1)

    c(pval,chibp,ffollp,lval,spval,mu[xm])
  })
  rbind(sim %>% 
        select(-simvals) %>% 
        mutate(pvals=1-pvals) %>% 
        mutate(method='condest'),
        data.frame(t(mhtpvals)) %>%
        setNames(c('mht','chibs','follman','loglog','spa','popvals')) %>%
...
#set.seed(1234)
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,nsim=200,mutype='spread')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,nsim=200,mutype='onegood')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,nsim=200,mutype='equal')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,nsim=200,mutype='halfgood')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,rho=0.7,nsim=200,mutype='onegood')
#mhtsim(nday=252*4,nstrat=200,one_zeta=0.10,nsim=200,mutype='halfgood')
#mhtsim(nday=252*4,nstrat=20,one_zeta=0.10,nsim=200,mutype='halfgood')
#mhtsim(nday=252*4,nstrat=10,one_zeta=0.10,rho=0.7,nsim=200,mutype='onegood')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0.10,nsim=200,rho=0.99,mutype='equal')
#mhtsim(nday=252*4,nstrat=100,one_zeta=0,nsim=200,rho=0.99,mutype='equal')
#mhtsim(nday=252*4,nstrat=200,one_zeta=0,nsim=200,rho=0.99,mutype='equal')
#mhtsim(nday=252*4,nstrat=200,one_zeta=0,nsim=200,rho=0.999,mutype='equal')
@
...
<<'power_plotz_one',dependson=c('sims_vs_mht'),echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=5.5,out.width="0.975\\textwidth",fig.pos='h',fig.cap=fig_cap>>=
...
We note that the chi-bar-square test appears to have higher power than either
...
# set up the functions
rawsim <- function(nday,nstrat,nsim=100,rho=0) {
  R <- pmin(diag(nstrat) + rho,1)  
  mu <- rep(0,nstrat)
  ...
  mhtpvals <- replicate(nsim,{
		...
		bonf_pval <- nstrat * SharpeR::psr(max(x),df=nday-1,zeta=0,ope=1,lower.tail=FALSE) 
		# do the correction
    ...
    # follman test
    g2 <- f1 * (zetavg - psnr0)^2 + f2 * sum((x - zetavg)^2)
    follp <- pchisq(g2,df=nstrat,lower.tail=FALSE)
    ffollp <- ifelse(zetavg > psnr0,0.5 * follp,1 - 0.5*follp)

    c(bonf_pval,corr_pval,chibp,ffollp)
  })
  data_frame(bonf_pvals=as.numeric(mhtpvals[1,]),
						 corr_pvals=as.numeric(mhtpvals[2,]),
             chib_pvals=as.numeric(mhtpvals[3,]),
             foll_pvals=as.numeric(mhtpvals[4,]),
             nelc_pvals=1-sim$pvals)
}
many_rawsim <- function(nday,nstrat,rho,nsim=1000L,nnodes=7) {
  if ((nsim > 10*nnodes) && require(doFuture)) {
		registerDoFuture()
		plan(multiprocess)
		nper <- as.numeric(table(1:nsim %% nnodes))
    retv <- foreach(iii=1:nnodes,.export = c('nday','nstrat','rho','nper','rawsim')) %dopar% {
			rawsim(nday=nday,nstrat=nstrat,rho=rho,nsim=nper[iii])
    } %>%
      bind_rows()
  } else {
		retv <- rawsim(nday=nday,nstrat=nstrat,rho=rho,nsim=nsim)
  }
  retv
}
mhtsim <- function(alpha=0.05,...) {
...
rhoresu <- params %>%
  group_by(rho) %>%
    summarize(resu=list(mhtsim(nday=mht_nday,nstrat=mht_nstrat,rho=rho,nsim=mht_null_nsim))) %>%
  ungroup() %>%
  unnest()
#})
@

On the other hand, the na{\"i}ve MHT test cannot maintain the nominal type I rate in the face of correlated assets.
...
fig_cap <- paste0('The empirical type I rate under the null hypothesis is plotted versus $\\rho$ ',
                  ...
                  "for five testing procedures: the conditional estimation, the chi-bar square test, Follman's test, the vanilla Bonferroni correction, ",
...
ph <- rhoresu %>%
  mutate(method=case_when(grepl('^nelc_pvals$',method)    ~ 'cond. est.',
                          ...
                          grepl('^foll_pvals$',method)    ~ 'Follman',
                          TRUE ~ 'ERROR')) %>%
