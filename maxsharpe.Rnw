...
we should be concerned about \ssiz, \nstrat, the correlation of returns (which we do not even consider here),
...
rather we anticipate that the elements of \pvsnr will have be bell-shaped or even flat.
We simulate these cases, estimating the power of the various tests as a function of the \txtSNR of the selected strategy.
...
Thus the likelihood of making this kind of meta type I error when using MHT tests is perhaps low,
it is not zero.
...
fig_cap <- paste0("The empirical power of the conditional estimation, MHT corrected test, chi-bar-square test, Follman's test, Hansen's SPA and corrected chi-bar-square procedures ",
                  ...
                  "We perform ",mht_nsim," simulations with $\\ssiz=",mht_nday,"$, $\\nstrat=",mht_nstrat,"$. ",
