...
We repeat this experiment 1000 times and collect the 1000
p-values. 
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_motherload_sims_log_plotz-1} \caption{We repeat the P-P plot of \figref{motherload_sims_hidden_plotz}, but in log-log space.  To check uniformity in both tails we compute $p_{*} = \log{2\min\wrapParens{p,1-p}}$, and plot against a log uniform law. Again, we plot the deviance of sample quantiles from the theoretical quantiles, with error bars around the $x$ axis. \cite{velez2015modified} Simulations use the exact \RMAT and \pvsnr to compute the covariance matrix. }\label{fig:motherload_sims_log_plotz}
\end{figure}
...
$500$ simulations, computing p values from the feasible estimator.
...
Again we perform $100$ simulations with
$\nstrat=100$,
$\RMAT=\makerho{0.7}{0.3}$,
$\ssiz=1260\dayto{}$, \etc
...
For each setting we perform $100$ simulations then 
...
We have performed
$100$ simulations for each setting of the `good'
...
occuring even once over the $100$ simulations performed
for each setting. 
...
and $\psnr=\vzero$.
Again we consider $\nstrat=100$, 
$\ssiz=1008$, and perform $50$ simulations to estimate the 
