...
We repeat this experiment \ensuremath{10^{4}} times and collect the resultant putative p-values.
...
performing $\ensuremath{10^{4}}$ simulations, 
setting $\nstrat=100$,
$\RMAT=\makerho{0.7}{0.3}$.
$\ssiz=1260\dayto{}$, \etc
...
$\ensuremath{10^{4}}$ simulations, computing p values from the feasible estimator.
...
Again we perform $1000$ simulations with
$\nstrat=100$,
$\RMAT=\makerho{0.7}{0.3}$,
$\ssiz=1260\dayto{}$, \etc
...
$28$ assets, with $\ssiz=328$.
For each setting we perform $1000$ simulations then 
...
We have performed
$1000$ simulations for each setting of the `good'
...
occuring even once over the $1000$ simulations performed
for each setting. 
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_power_plotz_two-1} \caption[The empirical power of the conditional estimation, MHT corrected test, chi-bar-square test, Follman's test, Hansen's SPA and corrected chi-bar-square procedures are shown versus the \txtSNR of the asset with maximum \txtSR for the uniform and bell-shaped arrangements of the vector \pvsnr]{The empirical power of the conditional estimation, MHT corrected test, chi-bar-square test, Follman's test, Hansen's SPA and corrected chi-bar-square procedures are shown versus the \txtSNR of the asset with maximum \txtSR for the uniform and bell-shaped arrangements of the vector \pvsnr. We perform 5,000 simulations with $\ssiz=328$, $\nstrat=33$. Cases with low incidence were removed as the simulated rejection rates are not representative. }\label{fig:power_plotz_two}
\end{figure}
...
and $\psnr=\vzero$.
We set $\nstrat=20$, $\ssiz=504$, 
and perform $1000$ simulations to estimate the empirical rejection rate.
