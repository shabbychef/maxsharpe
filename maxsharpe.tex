...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_motherload_sims_log_plotz-1} \caption{We repeat the P-P plot of \figref{motherload_sims_hidden_plotz}, but plot quantiles of the log p values against a log uniform law. Again, we plot the deviance of sample quantiles from the theoretical quantiles, with error bars around the $x$ axis. \cite{velez2015modified} Simulations use the exact \RMAT and \pvsnr to compute the covariance matrix. The plot indicates that the smallest p-values reported by the procedure are slightly smaller than they theoretically should be; given the log transform, one can interpret the plot as indicating the nominal p-values are around 3\% smaller than they should be. }\label{fig:motherload_sims_log_plotz}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_causal_tfive_sims_plotz-1} \caption[The computed p-values from the conditional estimation procedure over 10000 simulations are logged, then plotted against a log uniform law using the subsampling procedure]{The computed p-values from the conditional estimation procedure over 10000 simulations are logged, then plotted against a log uniform law using the subsampling procedure. Returns are drawn from a $\tstat{}\wrapParens{5}$ distribution.  Sample estimates are used to construct the variance-covariance matrix of \svsr.  The experiments are performed twice: first assuming that returns are Gaussian; then assuming returns are elliptical with unknown kurtosis factor that we estimate from the sample.  Both procedures give near-uniform p values. Without correcting for higher order moments, the nominal p values are smaller than uniform, on the order of 6\% larger; under the correction, however, the nominal p values are larger than uniform, by as much as 3\%. }\label{fig:causal_tfive_sims_plotz}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_power_plotz_one-1} \caption[The empirical power of the conditional estimation, MHT corrected test, and one-sided test are shown versus the \txtSNR of the asset with maximum \txtSR under different arrangements of the vector \pvsnr]{The empirical power of the conditional estimation, MHT corrected test, and one-sided test are shown versus the \txtSNR of the asset with maximum \txtSR under different arrangements of the vector \pvsnr.}\label{fig:power_plotz_one}
\end{figure}
...
%These consist of 
We consider the 1080 months of data on five industries
from 1927 to 2016.917.
...
The industry portfolio with the highest \txtSR was
 with a \txtSR of 
around $0.192\,\moto{-\halff}$ which is approximately
$0.664\,\yrto{-\halff}$.

% latex table generated in R 3.4.3 by xtable 1.8-2 package
% Fri Nov 22 05:48:23 2019
\begin{table}[ht]
\centering
\begin{tabular}{r|c}
  \hline
industry & Sharpe Ratio \\ 
  \hline
Healthcare & $ 0.192\,\moto{-\halff} $ \\ 
  Consumer & $ 0.188\,\moto{-\halff} $ \\ 
  Manufacturing & $ 0.174\,\moto{-\halff} $ \\ 
  Technology & $ 0.169\,\moto{-\halff} $ \\ 
  Other & $ 0.141\,\moto{-\halff} $ \\ 
   \hline
\end{tabular}
...
intervals on the \txtSNR of the  portfolio. 
...
$\left[0.12\,\moto{-\halff}, \infty\right)$.
...
This yields the confidence interval 
$\left[0.039\,\moto{-\halff}, \infty\right)$.
