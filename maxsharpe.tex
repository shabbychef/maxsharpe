...
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_motherload_sims_hidden_plotz-1} \caption{The computed p-values from the conditional estimation procedure over 1000 simulations are compared to a uniform law. Given the large sample size, a Q-Q plot is visually hard to interpret. Instead we plot the deviance of sample quantiles from the theoretical quantiles, with error bars around the $x$ axis, as suggested by V{\'e}lez and Correa Morales. \cite{velez2015modified} Simulations use the exact \RMAT and \pvsnr to compute the covariance matrix. The plot supports uniformity of the putative p-values. }\label{fig:motherload_sims_hidden_plotz}
\end{figure}
