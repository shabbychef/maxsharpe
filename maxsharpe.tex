...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_motherload_sims_log_plotz-1} \caption{We repeat the P-P plot of \figref{motherload_sims_hidden_plotz}, but in log-log space.  To check uniformity in both tails we compute $p_{*} = \logof{2\min\wrapParens{p,1-p}}$, and plot against a log uniform law. Again, we plot the deviance of sample quantiles from the theoretical quantiles, with error bars around the $x$ axis. \cite{velez2015modified} Simulations use the exact \RMAT and \pvsnr to compute the covariance matrix. The plot indicates that the p-values reported by the procedure are slightly smaller than they theoretically should be; given the log transform, one can interpret the plot as indicating the nominal p-values are around 1-2\% smaller than they should be. }\label{fig:motherload_sims_log_plotz}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.975\textwidth]{figure/maxsharpe_causal_tfive_sims_plotz-1} \caption{The computed p-values from the conditional estimation procedure over 10000 simulations are transformed via $p_{*} = \logof{2\min\wrapParens{p,1-p}}$, then plotted against a log uniform law using the subsampling procedure. Returns are drawn from a $\tstat{}\wrapParens{5}$ distribution.  Sample estimates are used to construct the variance-covariance matrix of \svsr.  The experiments are performed twice: first assuming that returns are Gaussian; then assuming returns are elliptical with unknown kurtosis factor that we estimate from the sample.  Both procedures give near-uniform p values. Without correcting for higher order moments, the nominal p values are larger than uniform, on the order of 5\% larger; under the correction, however, the nominal p values are smaller than uniform, by as much as 10\%. }\label{fig:causal_tfive_sims_plotz}
\end{figure}
